    ...
    public class TamperProofTask : ICloakTask
    {
...
#else
            string resourceNamespace = context.NameManager.GenerateName(NamingType.Type);
#endif
...
#else
                string resourceName = resourceNamespace + "." + context.NameManager.GenerateName(NamingType.Type);
#endif
        ...
        private static void BuildBootstrapper(ICloakContext context, AssemblyDefinition assembly, string passwordKey, Guid salt)
        {
...
#else
            TypeDefinition programType = new TypeDefinition(context.NameManager.GenerateName(NamingType.Type),
                                                        context.NameManager.GenerateName(NamingType.Type),
                                                        TypeAttributes.NotPublic | TypeAttributes.Class |
                                                        TypeAttributes.AutoClass | TypeAttributes.AnsiClass |
                                                        TypeAttributes.Serializable | TypeAttributes.BeforeFieldInit,
                                                        assembly.Import(typeof(object)));
...
#else
            string assembliesLoadedVariableName = context.NameManager.GenerateName(NamingType.Field);
            string assemblyLockVariableName = context.NameManager.GenerateName(NamingType.Field);
            string executingAssemblyVariableName = context.NameManager.GenerateName(NamingType.Field);
            string loadedAssembliesVariableName = context.NameManager.GenerateName(NamingType.Field);
#endif
            ...
            TypeDefinition entryType = new TypeDefinition("Program",
                                            ...
                                            assembly.Import(typeof(object)));
#else
            TypeDefinition entryType = new TypeDefinition(context.NameManager.GenerateName(NamingType.Type),
                                                        context.NameManager.GenerateName(NamingType.Type),
                                                        TypeAttributes.NotPublic | TypeAttributes.Class |
                                                        TypeAttributes.AutoClass | TypeAttributes.AnsiClass |
                                                        TypeAttributes.BeforeFieldInit,
...
#else
            MethodDefinition method = new MethodDefinition(context.NameManager.GenerateName(NamingType.Method),
                                               MethodAttributes.Private | MethodAttributes.HideBySig, assembly.Import(typeof(void)));
#endif
...
#else
            MethodDefinition entryPoint =
                new MethodDefinition(context.NameManager.GenerateName(NamingType.Method),
                                     MethodAttributes.Private | MethodAttributes.Static |
                                     MethodAttributes.HideBySig, assembly.Import(typeof(void)));
#endif
...
#else
            il.Append(il.Create(OpCodes.Ldstr, context.NameManager.GenerateName(NamingType.Type)));
#endif
